...
class Gene {
	...
	function __construct($gene_id, $chr = null) {
		$sql = "select * from gene where (UPPER(gene) ='$gene_id' or UPPER(symbol) ='$gene_id') and target_type='ensembl' and species = 'hs'";
		if ($chr != null)
		...
		$ensembl_id = null;
		if (count($rows) == 0) {
			$sql = "select * from gene g where exists(select * from gene_alias a where (g.symbol=a.symbol or g.symbol=a.gene_synonym) and (UPPER(a.symbol)='$gene_id' or UPPER(a.gene_synonym)='$gene_id')) and target_type='ensembl' and species = 'hs'";
			$rows = DB::select($sql);
		}
		if (count($rows) > 0) {
