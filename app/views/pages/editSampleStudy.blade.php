...
<script type="text/javascript">
	...
	var study_id = null;
	var tab_width = null;
	var sample_list = {{json_encode($sample_list)}};   
	...
	$(document).ready(function() { 
		...
		$('#sampleClear').click(function(){
			$('#selectSamples').structFilter("clear");
		}); 

		$("#selectSamples").on("submit.search", function(event){
			document.getElementById('filterValue').value = JSON.stringify($('#selectSamples').structFilter("val"), null, 2);
			$('#filterForm').submit();
		});

		$('#sample_tree').on('select_node.jstree', function (e, data) {	
			var key = data.node.id; 
			parent = $('#sample_tree').jstree(true).get_parent(data.node); 
			if (sample_list[key] != null) {
				$('#lstAvail').empty();
				$('#lblAvail').text('Selected diagnosis: ' + data.node.text);
				for(var d=0;d<sample_list[key].length;d++) {                         
					var value = sample_list[key][d];                            
		...
		var current_node = $('#sample_tree').jstree(true).get_node(current_node_id);
		var current_tissue = current_node.text;
		var parent_id = $('#sample_tree').jstree(true).get_parent(current_node);
		...
		var root = $('#sample_tree').jstree(true).get_node('root'); 
		enable_node(root);
		...
		$('#lblPlatform').text('');
		$('#lblLibType').text('');
		$('#lblSelected').text('Selected samples (0)');
	      });
		...
		$('#sample_tree').bind('ready.jstree', function(e, data) {
			updateType();
		});
		...
		var sample_data = {};
		var groups = []; 
		sample_data.name = $('#studyName').val();
		sample_data.id = study_id;
		sample_data.mode = '{{$mode}}';
		sample_data.study_type = $('#lblType').text();
		sample_data.study_type_code = current_type;
		sample_data.material_type = $('#lblMaterialType').text();
		sample_data.exp_type = $('#lblExpType').text();
		sample_data.platform = $('#lblPlatform').text();
		sample_data.library_Type = $('#lblLibType').text();
		sample_data.study_desc = $('#studyDescription').val();
		sample_data.is_public = $('#isPublic').prop('checked').toString();
		sample_data.groups = groups;
		var tabs = $('#tabGroups').tabs('tabs');
		for (var i=0;i<tabs.length;i++) {
		     ...
		     $('#' + list_id + ' option').each(function(j){
		           var sample = {"id":$(this).val()};
		           samples.push(sample);
		     });
		     groups.push(group);
		}             
		var json_string = JSON.stringify(sample_data);
		...
		return true;
	      ...
	      $('#lblAvail').text('Available samples (0)');
	      $('#lblSelected').text('Selected samples (0)');
	      $('#lblType').text('No selected type');      

	      if (mode == 'edit') {		
			study_id = study.id;
			$('#lblType').text(study.study_type);
			$('#lblMaterialType').text(study.material_type);
			$('#lblExpType').text(study.exp_type);
			$('#lblPlatform').text(study.platform);
			$('#lblLibType').text(study.library_type);
			...
			for (var i=0;i<study.tissues.length;i++) {
				...
				for (var j=0;j<study.tissues[i].samples.length;j++) {
					var tab_id = 'tab_' + (tab_counter - 1).toString();
					var list_id = tab_id + '_lstSelected';
					$('#' + list_id).append(new Option(study.tissues[i].samples[j]));
				}
			}
	    ...
	    function updateType() {
	      ...
	      $('#sample_tree').jstree(true).enable_node(current_node);
	      parent = $('#sample_tree').jstree(true).get_parent(current_node); 
	      while (parent != 'root') {                        
		      current_node = $('#sample_tree').jstree(true).get_node(parent);
		      $('#sample_tree').jstree(true).enable_node(current_node);    
		      parent = $('#sample_tree').jstree(true).get_parent(current_node);               
	      }      
	    }
	    ...
	    function updateLabel() {         
	      var list_id = getCurrentList();         
	      $('#lblSelected').text('Selected samples (' + $('#' + list_id + ' option').size().toString() + ')');
	    ...
	    function disable_node(node, type) {
	      $.each(node.children, function(index, child) {
		    $('#sample_tree').jstree(true).disable_node(child);
		    if ($('#sample_tree').jstree(true).is_parent(child)) {
		        child_node = $('#sample_tree').jstree(true).get_node(child);
	    ...
	    function enable_node(node) {
	      $.each(node.children, function(index, child) {
		    $('#sample_tree').jstree(true).enable_node(child);
		    if ($('#sample_tree').jstree(true).is_parent(child)) {
		        child_node = $('#sample_tree').jstree(true).get_node(child);
...
table.pretty tbody td {
 ...
 <div id="out_container" class="easyui-panel" style="width:100%;height:100%;padding:10px;">
  <div class="easyui-layout" data-options="fit:true">
    <div data-options="region:'west',split:true" style="width:300px;padding:10px;overflow:auto;" title="Choose tissuies">
        <div id="sample_tree"></div>   
    </div>
    <div data-options="region:'center'" style="padding:0px; border:0px">
       <div class="easyui-panel" style="height:100%;padding:0px;">
            <div class="easyui-layout" data-options="fit:true">
                  <div class="easyui-panel" data-options="region:'north',split:true" style="height:220px;padding:10px;overflow:auto;" title="Study information">
                       <table cellpadding="0" cellspacing="0" border="1" class="pretty" word-wrap="break-word" id="tblDetail" style='width:100%'>
                            <tr><th>Study type:</th><td> <div id="lblType" text="test"></div></td><th>Study name:</th><td> <input id='studyName' size="50" type="text"></td></tr>  
                            <tr><th>Material type:</th><td> <div id="lblMaterialType" style="text-align:left;" text="test"></div></td><th rowspan=5>Study Desc:</th><td rowspan=4> <textarea id='studyDescription' rows="5" cols="50"></textarea> </td></tr>
                            <tr><th>Experiment type:</th><td> <div id="lblExpType" style="text-align:left;" text="test"></div></td></tr>
                            <tr><th>Platform:</th><td> <div id="lblPlatform" text="test"></div></td></tr>
			    <tr><th>Library type:</th><td> <div id="lblLibType" text="test"></div></td></tr>
                            <tr><th>Make Public? </th><td> <input id='isPublic' class="onco_checkbox" type='checkbox' checked/></td><td><a href="#" id="btnClearAll" class="btn btn-primary" data-options="iconCls:'icon-cancel'">Clear all</a></td></tr>
                       </table> 
                  </div>
                  <div class="easyui-panel" data-options="region:'center',split:true" style="width:100%;padding:10px;overflow:auto;" title="Choose samples">
				...
				<tr>
					<td valign="top">
						<div id="tabSource" class="easyui-tabs" data-options="pill:true,narrow:true" style="width:400px;height:100%;padding:0px">               
							<div title="Samples in tissue" data-options="iconCls:'icon-ok'" style="height:100%">
								<select name="lstAvail" id="lstAvail" size="4" multiple="multiple" style="height:100%;width:400px;"> </select> 
							</div>
						</div>
					</td> 
					<td>
						<input type="button" class="btn btn-primary btn-block btnEdit" id="btnRightAllGroup" value="Add Tissue" style="display:block;" />
						<input type="button" class="btn btn-primary btn-block btnEdit" id="btnRightAll" value=">>&nbsp;" style="display:block;" />
						<input type="button" class="btn btn-primary btn-block btnEdit" id="btnRight" value="&nbsp;>&nbsp;" style="display:block;" />
						<input type="button" class="btn btn-primary btn-block btnEdit" id="btnLeft" value="&nbsp;<&nbsp;" style="display:block;" />
						<input type="button" class="btn btn-primary btn-block btnEdit" id="btnLeftAll" value="<<&nbsp;" style="display:block;" />
					</td>
					<td valign="top">               
