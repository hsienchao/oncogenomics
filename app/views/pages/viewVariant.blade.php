...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$("#btnAnnoSearch").on('click', function(){
			url_insert="{{url('/insertVariant')}}"+"/"+chr+"/"+start+"/"+end+"/"+ref+"/"+alt
			$.ajax({ url: url_insert, async: true, dataType: 'text', success: function(d) {
				alert ('variant has ben sent to avia for annotation');
				}
			});
    		
		});
		url="{{url('/getVarAnnotationByVariant')}}"+"/"+chr+"/"+start+"/"+end+"/"+ref+"/"+alt
		...
		$.ajax({ url: url, async: true, dataType: 'text', success: function(d) {
				...
				if (json_data.data.length == 0) {
					$("#lblMessage").text("No annotated variant found! would you want to annotate it?");
	...
	function doFilter() {
    	...
    	if (first_loading) {
    		...
    		diagnoses.sort().forEach(function(d){
    			if (diagnosis == d)
	...
	function showTable(data) {
		...
		*/
		...
		@if ($type == 'rnaseq' || $type == "variants" || $type == "hotspot") 
			tier_html += '<select id="tier_type" ><option value="tier_or">Germline or somatic</option><option value="tier_and">Germline and somatic</option><option value="germline_only">Germline tier only</option><option value="somatic_only">Somatic tier only</option></select>';
		@endif
		tier_html +='<td><span class="btn-group" id="tiers" data-toggle="buttons">' +
					...
					'</span>';
		if (type == "germline")
			tier_html +='&nbsp;<a href="{{url('/images/ACMG.png')}}" title="ACMG definitions" class="fancybox mytooltip acmg_definition"><img src={{url("images/help.png")}}></img></a>' + 
					...
					'</span>';
		if (gene_id != 'null') {
			tier_html += '&nbsp;Patients:&nbsp;<select id="selPatients" width="100px" ><option value="any">Any</option></select>';
			tier_html += '&nbsp;Diagnosis:&nbsp;<select id="selDiagnosis" width="100px" ><option value="any">Any</option></select>';
			tier_html += '&nbsp;AA pos:&nbsp;<select id="selAAPos" width="50px" ><option value="any">Any</option></select>';
		}
		...
		$("div.toolbar").html('<div><table style="font-size:13"><tr>' + tier_html + 
								...
								'Select Columns</button>&nbsp;Annotation: <span style="color:red">' + 
		...
		tbl.columns().iterator('column', function ( context, index ) {
			...
			col_html += '<input type=checkbox ' + checked + ' class="onco_checkbox" id="data_column" value=' + index + '><font size=3>&nbsp;' + column_text + '</font></input><BR>';
		} );
		...
		$.fn.dataTableExt.afnFiltering.push( function( oSettings, aData, iDataIndex ) { 
			...
			var is_hotspot = (aData[hotspot_site_idx] != '');
				var caller = aData[caller_idx];
				var fisher_score = aData[fisher_score_idx];
				var exp_type = aData[exp_type_idx];
				var normal_cov_val = parseInt(aData[normal_cov_idx]);
...
//							console.log("FAILED PANEL");
//							console.log("total coverage "+total_cov_val+" "+high_conf_setting.somatic_panel_total_cov);
//							console.log("normal coverage "+normal_cov_val+" "+high_conf_setting.somatic_panel_normal_total_cov);
//							console.log("vaf "+vaf_val+" "+high_conf_setting.somatic_panel_vaf);
			...
			if (final_decision && select_all && status != 'closed') {
				var_list.push(var_id);				
			}
	...
	function showMutalyzer(chr, start_pos, end_pos, ref, alt, gene, transcript) {
		...
		$.ajax({ url: url, async: true, dataType: 'text', success: function(data) {
				...
				console.log('https://mutalyzer.nl/name-checker?description=' + hgvs_string );
				//mutalyzer
				$.fancybox({
        			'type' : 'iframe',
        			'width'     : 1200,
    ...
    <div rel="body" style="text-align:left;">
    	<div class="container-fluid">
        	...
        	<div class="row">        		
        		<div class="col-md-7">
        			<H4>
        			<a width="500" href="{{url('/images/ACMG.png')}}" title="ACMG definitions" class="fancybox mytooltip acmg_definition"><img src="{{url('/images/help.png')}}"></a>&nbsp;
...
<div id="pop_var_details" style="display: none; width:85%;min-height:90%;overflow: auto; background-color=white;padding:10px">    
    <div rel="body" style="text-align:left;">
    	...
    	<div id='table_area'>
    		<div class="container-fluid" style="padding:10px">
    			<table class="table table-bordered table-hover" width="100%"><tr>
    				<td>
    					<H5>Patient&nbsp;:&nbsp;<lable id="lblVarDetailPatientID" style="color: red;"></lable></H5>
    				</td>
    				<td>
    					<H5>Variants&nbsp;:&nbsp;<lable id="lblVarDetailVar" style="color: red;"></lable> in gene <lable id="lblVarDetailGene" style="color: red;"></lable></H5>
    				</td>
    				<td style="text-align:right;">
...
<div id ='var_not_found' style='margin-left: 2%;display:none'>
	<lable id="lblMessage" style="font-size: 200%;"> </lable></br>
	<span>Chromosome:  {{$chr}}</span></br>
    <span>Start:  {{$start}}</span></br>
    <span>End:  {{$end}}</span></br>
    <span>Ref:  {{$ref}}</span></br>
    <span>Alt:  {{$alt}}</span></br></br>
	
	<button display='none' id="btnAnnoSearch" type="button" class="btn btn-info">Annotate</button></br>
	<span>*Note: Please allow 24 hours for Avia to annotate
</div>
