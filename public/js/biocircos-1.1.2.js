  ...
  BioCircos.prototype.draw_genome = function(genome){
    ...
    if(self.SNP.length > 0){
        ...
        for(var snpi=0; snpi<self.SNP.length; snpi++){
            ...
            // todo incorporate snp labels on the outside of the cytoband
            if(self.SNPsettings.displaySnpOuterLabel==true){
                svg.append("g")
                    .attr("class", "BioCircosSNPOuterLabel")
                    .selectAll("text")
                    .data(link_objects)
                    .enter().append("text")
                    .attr("transform", function(d) {
            ...
            function BioCircosLINK(d) {
              return self.LINK[linki].map(function(v, i) {
                ...
                return {
                  ...
                  link_X1_on_cyto: (0 + Math.sin((v.g1start/2+v.g1end/2) * start_k + d[self.initGenome[v.g1chr]].startAngle) * (self.outerRadius)),
                  link_Y1_on_cyto: (0 - Math.cos((v.g1start/2+v.g1end/2) * start_k + d[self.initGenome[v.g1chr]].startAngle) * (self.outerRadius)),
                  link_X2_on_cyto: (0 + Math.sin((v.g2start/2+v.g2end/2) * end_k + d[self.initGenome[v.g2chr]].startAngle) * (self.outerRadius)),
                  link_Y2_on_cyto: (0 - Math.cos((v.g2start/2+v.g2end/2) * end_k + d[self.initGenome[v.g2chr]].startAngle) * (self.outerRadius)),
                  link_X1_pad_cyto: (0 + Math.sin((v.g1start/2+v.g1end/2) * start_k + d[self.initGenome[v.g1chr]].startAngle) * (self.LINKsettings.LinkRadius+self.LINKsettings.LinkLabelPad)),
                  link_Y1_pad_cyto: (0 - Math.cos((v.g1start/2+v.g1end/2) * start_k + d[self.initGenome[v.g1chr]].startAngle) * (self.LINKsettings.LinkRadius+self.LINKsettings.LinkLabelPad)),
                  link_X2_pad_cyto: (0 + Math.sin((v.g2start/2+v.g2end/2) * end_k + d[self.initGenome[v.g2chr]].startAngle) * (self.LINKsettings.LinkRadius+self.LINKsettings.LinkLabelPad)),
                  link_Y2_pad_cyto: (0 - Math.cos((v.g2start/2+v.g2end/2) * end_k + d[self.initGenome[v.g2chr]].startAngle) * (self.LINKsettings.LinkRadius+self.LINKsettings.LinkLabelPad))
                };
              });
            }
        for(var linki=0; linki<self.LINK.length; linki++) {
            ...
            if (self.LINKsettings.displayLinkLabel == true) {
                ...
                svg.append("g")
                    ...
                    .attr("class", "BioCircosLINKLabel")
                    // 0.048 is the threshold of closeness before we display a fusion link in one line instead of at each end TODO: Make this a ratio or break it out into a variable
                    // TODO maybe break out below calculation into a function
                    .attr("id", function (d) {
                        return (Math.abs((Math.round(d.link_angle1 * 100) / 100) - (Math.round(d.link_angle2 * 100) / 100)) < 0.048) ? '' : d.link_label1;
                    })
                    .attr("x", self.LINKsettings.LinkLabelPad)
                    .attr("dy", ".35em")
                    .attr("fill", self.LINKsettings.LinkLabelColor)
                ...
                svg.append("g")
                    ...
                    .attr("class", "BioCircosLINKLabel")
                    .attr("id", function (d) {
                        return (Math.abs((Math.round(d.link_angle1 * 100) / 100) - (Math.round(d.link_angle2 * 100) / 100)) < 0.048) ? d.link_pair : d.link_label2;
                    })
                    .attr("x", self.LINKsettings.LinkLabelPad)
                    .attr("dy", ".35em")
                    .attr("fill", self.LINKsettings.LinkLabelColor)
            ...
            // Mapping coordinates for lines from cytoband fusion point to fusion label

            //todo : maybe don't plot the points but instead make an array of coordinates then plot wavy lines

            // svg.append("g")
            //     .attr("class", "BioCircosFusionLabelLinkInner1")
            //     .selectAll("text")
            //     .data(link_objects)
            //     .enter().append("circle")
            //     .attr("cx", function (d) {
            //         return d.link_X1_on_cyto;
            //     })
            //     .attr("cy", function (d) {
            //         return d.link_Y1_on_cyto;
            //     })
            //     .attr("r", 1)
            //     .style("fill", "red");
            //
            // svg.append("g")
            //     .attr("class", "BioCircosFusionLabelLinkOuter1")
            //     .selectAll("text")
            //     .data(link_objects)
            //     .enter().append("circle")
            //     .attr("cx", function (d) {
            //         return d.link_X1_pad_cyto;
            //     })
            //     .attr("cy", function (d) {
            //         return d.link_Y1_pad_cyto;
            //     })
            //     .attr("r", 1)
            //     .style("fill", "red");
            //
            // svg.append("g")
            //     .attr("class", "BioCircosFusionLabelLinkInner2")
            //     .selectAll("text")
            //     .data(link_objects)
            //     .enter().append("circle")
            //     .attr("cx", function (d) {
            //         return d.link_X2_on_cyto;
            //     })
            //     .attr("cy", function (d) {
            //         return d.link_Y2_on_cyto;
            //     })
            //     .attr("r", 1)
            //     .style("fill", "blue");
            //
            // svg.append("g")
            //     .attr("class", "BioCircosFusionLabelLinkOuter2")
            //     .selectAll("text")
            //     .data(link_objects)
            //     .enter().append("circle")
            //     .attr("cx", function (d) {
            //         return d.link_X2_pad_cyto;
            //     })
            //     .attr("cy", function (d) {
            //         return d.link_Y2_pad_cyto;
            //     })
            //     .attr("r", 1)
            //     .style("fill", "blue");
            ...
            //     .style("fill", "red")
            //     .attr("transform", function (d) {
            //         return "rotate(" + (d.link_angle1 * 180 / Math.PI - 90) + ")translate(" + self.outerRadius + ")";
            //     });
            //
            // // fusion label point
            // svg.append("g")
            ...
            //     .style("fill", "red")
            //     .attr("transform", function (d) {
            //         return "rotate(" + (d.link_angle1 * 180 / Math.PI - 90) + ")translate(" + (self.LINKsettings.LinkRadius+self.LINKsettings.LinkLabelPad) + ")";
            //     });
            //
            // // fusion2 outer cytoband point
            // svg.append("g")
            //     .attr("class", "BioCircosFusionLabelLinkInner2")
            //     .selectAll("text")
            //     .data(link_objects)
            ...
            //     .style("fill", "blue")
            //     .attr("transform", function (d) {
            //         return "rotate(" + (d.link_angle2 * 180 / Math.PI - 90) + ")translate(" + self.outerRadius + ")";
            //     });
            //
            // // fusion2 label point
            // svg.append("g")
            ...
            //     .style("fill", "blue")
            //     .attr("transform", function (d) {
            //         return "rotate(" + (d.link_angle2 * 180 / Math.PI - 90) + ")translate(" + (self.LINKsettings.LinkRadius+self.LINKsettings.LinkLabelPad) + ")";
            //     });
