...
(function($){
  ...
  BioCircos = function(){
      ...
      if(arguments.length >= 2){
          ...
          for (var n=0; n< arguments.length; n++){
              var reg=/^LEGEND/;
              if(reg.test(arguments[n][0])){
                  self.LEGEND.push(arguments[n][1]);
              }
          }
      ...
      self.SNPsettings = {
          "maxRadius": 200,
          "minRadius": 190,
          "yAxisRange": false, // if false auto fit range else [yMin,yMax]
          "SNPFillColor": "red",
      ...
      self.showLegend = self.settings.showLegend;
      ...
      self.ticksInterval=self.settings.ticks.interval;
      self.ticksSuffix=self.settings.ticks.suffix;
      ...
      self.genomeTextWeight=self.settings.genomeLabel.textWeight;
      self.genomeTextOrientUpright=self.settings.genomeLabel.orientUpright;
  ...
  BioCircos.prototype.init_SNPsettings = function(){
    ...
    self.SNPsettings = {
          "maxRadius": 200,
          "minRadius": 190,
          "yAxisRange": false, // if false auto discern range else [yMin,yMax]
          "SNPFillColor": "red",
          "PointType": "circle",
          "LinkWidth": 3,
          "LinkAxisPad": 3,
  ...
  BioCircos.prototype.snp_value_maxmin = function(snpIn){
      ...
      if (self.SNPsettings.yAxisRange !== false){
          snpValueMaxmin[0]=self.SNPsettings.yAxisRange[1];
          snpValueMaxmin[1]=self.SNPsettings.yAxisRange[0];
      }else{
          var i=snpIn.length;
          ...
          if (snpValueMax === snpValueMin){
              snpValueMaxmin[0]=Math.ceil(snpValueMax);
              snpValueMaxmin[1]=Math.floor(snpValueMax);
          }else{
  ...
  BioCircos.prototype.histogram_value_maxmin = function(histogramIn){
      ...
      if (self.HISTOGRAMsettings.yAxisRange !== false){
          histogramValueMaxmin[0]=self.HISTOGRAMsettings.yAxisRange[1];
          histogramValueMaxmin[1]=self.HISTOGRAMsettings.yAxisRange[0];
      }else{
  ...
  BioCircos.prototype.line_value_maxmin = function(lineIn){
      ...
      if (self.LINEsettings.yAxisRange !== false){
          lineValueMaxmin[0]=self.LINEsettings.yAxisRange[1];
          lineValueMaxmin[1]=self.LINEsettings.yAxisRange[0];
      }else{
          var i=lineIn.length;
  ...
  BioCircos.prototype.draw_genome = function(genome){
    ...
    if(self.genomeTextDisplay == true){
        svg.append("g").selectAll("text")
          ...
          .enter().append("text")
            ...
            .style("font-weight", self.genomeTextWeight)
	    .each( function(d,i) { 
        ...
        .attr("text-anchor", function(){
            if (self.genomeTextOrientUpright == true){return "middle";}
        })
	    .attr("transform", function(d){
	        if (self.genomeTextOrientUpright == false){
                return "rotate(" + ( d.angle * 180 / Math.PI ) + ")" +
                    "translate(0,"+ -1.0*(outerRadius+10) +")" +
                    ( ( d.angle > Math.PI*2 && d.angle < Math.PI*0 ) ? "rotate(180)" : "");
            }else{
                var dist   = outerRadius+35;
    ...
    if(self.ticksDisplay == true){
        function groupTicks(d) {
          ...
          return d3.range(0, d.value, self.ticksScale).map(function(v, i) {
              return {
                  angle: (i % self.ticksInterval) ? null : v * k + d.startAngle,
                  label: (i % self.ticksInterval) ? null : v / self.ticksScale + ((typeof self.ticksSuffix != 'undefined') ? self.ticksSuffix : "")
              };
          });
        }
    ...
    //      .attr("y", d3.event.y );
    //}
    function findNewAngle(angle, offset) {
                  for (var i=0;i<self.angles.length;i++) {
                      if (Math.abs(self.angles[i] - angle) < offset) {
                            var s = (self.angles[i] > angle)? -1 : 1;              
                            new_angle = self.angles[i] + offset * s;
                            j = i + s;
                            while (j >=0 && j < self.angles.length) {                                
                              if (Math.abs(self.angles[j] - new_angle) > offset)
                                  return new_angle;
                              else {
                                  new_angle = self.angles[j] + offset * s;
                                  j = j + s;                  
                              }
                            }
                            return new_angle;                        
            ...
            function BioCircosHISTOGRAM(d) {
              return self.HISTOGRAM[histogrami].map(function(v, i) {
                ...
                return {
                  ...
                  histogram_value: v.value // * 2, SAH Removed because it was exceeding the top of the histogram
                };
              });
            }
    ...
    if(self.SNP.length > 0){
            function BioCircosSNP(d) {
              return self.SNP[snpi].map(function(v, i) {
                ...
                var label_line_start = self.settings.outerRadius + self.LINKsettings.LinkAxisPad + 38;  
                var label_line_end = self.settings.outerRadius + self.LINKsettings.LinkAxisPad + 53;                

                if (self.labels.indexOf(snp_label) != -1) {                  
                    snp_label = '';
                    label_line_start = label_line_end = 0;
                } else {
                    self.labels.push(snp_label);
                }                
                var angle = v.pos * snp_k + d[self.initGenome[v.chr]].startAngle;
                var ori_angle = angle;
                
                if (snp_label != '') {
                    angle = findNewAngle(angle, self.settings.textOffsetAngle);
                    self.angles.push(angle);
                    self.angles.sort(function(a, b){return a-b});
                }

                console.log(label_line_start);
                console.log(label_line_end);

                return {
                  snp_ori_angle: ori_angle,
                  snp_angle: angle,
                  snp_chr: v.chr,
                  snp_pos: v.pos,
                  snp_val: v.value,
                  ...
                  snp_label_x1 : (0 + Math.sin(ori_angle) * label_line_start),
                  snp_label_y1 : (0 - Math.cos(ori_angle) * label_line_start),
                  snp_label_x2 : (0 + Math.sin(angle) * label_line_end),
                  snp_label_y2 : (0 - Math.cos(angle) * label_line_end),
        ...
        for(var snpi=0; snpi<self.SNP.length; snpi++){
            ...
            svg.append("g")
              ...
              .selectAll("text")
                ...
                .attr("text-anchor", function(d) {
                    return ((d.snp_angle * 180 / Math.PI - 90) > 90) ? "end" : "start";
                })
                .attr("transform", function(d) {
                    var angle = (d.snp_angle * 180 / Math.PI - 90);
                    //var radius = self.SNPsettings.maxRadius+self.SNPsettings.LinkAxisPad;
                    var radius = self.settings.outerRadius + 47;
                    //console.log("snp:" + self.SNPsettings.maxRadius);
                    return "rotate(" + angle + ")translate(" + ( + (angle > 90 ? ((self.settings.outerRadius*2))-radius+112 : radius) )  + ")rotate(" + (angle > 90 ? -180 : 0) + ")";
                    // todo 110 in the above line is a calculation that needs to be figured out... I think
                })
                .attr("class", "BioCircosLINKLabel")
                .attr("id", function(d) { return d.snp_des; })
            ...
            .attr("class", "BioCircosSNP")
              .selectAll("path")
                .data(snp_objects)
                .enter().append("path")
                .attr("d", function(d) { console.log(d.snp_label_x1 + ' ' + d.snp_label_y1 + ' ' + d.snp_label_x2 + ' ' + d.snp_label_y2);  return "M" + (d.snp_label_x1) +"," + (d.snp_label_y1) +" L" + (d.snp_label_x2) +"," + (d.snp_label_y2) + " M" + (d.snp_label_x1) +"," + (d.snp_label_y1) +" L" + (d.snp_label_x2) +"," + (d.snp_label_y2); })
                .attr("class", "BioCircosSNP")
                .attr("fill","none")
                .attr("stroke",'gray')
                .attr("stroke-width",1);
    ...
    if(self.LINK.length > 0){
            function BioCircosLINK(d) {
              return self.LINK[linki].map(function(v, i) {
                ...
                var link_angle1 = (v.g1start/2+v.g1end/2) * start_k + d[self.initGenome[v.g1chr]].startAngle;
                var link_angle2 = (v.g2start/2+v.g2end/2) * end_k + d[self.initGenome[v.g2chr]].startAngle;
                var link_label1 = v.g1name;
                var link_label2 = v.g2name;
                //if (Math.round( link_angle1 * 10 ) / 10 == Math.round( link_angle2 * 10 ) / 10) {
                //  link_label1 = link_label1 + "-" + link_label2;
                //  link_label2 = '';
                //}                
                var fusion_label = v.fusion;
                var label1_line_start = self.settings.outerRadius + self.LINKsettings.LinkAxisPad + 40;
                var label1_line_end = self.settings.outerRadius + self.LINKsettings.LinkAxisPad + 55;
                var label2_line_start = self.settings.outerRadius + self.LINKsettings.LinkAxisPad + 40;
                var label2_line_end = self.settings.outerRadius + self.LINKsettings.LinkAxisPad + 55;
                
                if (self.labels.indexOf(link_label1) != -1) {                  
                  link_label1 = '';
                  label1_line_start = label1_line_end = 0;
                } else {
                  self.labels.push(link_label1);
                }                
                if (link_label2 == '' || self.labels.indexOf(link_label2) != -1) {
                  link_label2 = '';
                  label2_line_start = label2_line_end = 0;
                } else {
                  self.labels.push(link_label2);
                }

                var link_ori_angle1 = link_angle1;
                var link_ori_angle2 = link_angle2;
                
                if (link_label1 != '') {
                    link_angle1 = findNewAngle(link_angle1, self.settings.textOffsetAngle);
                    self.angles.push(link_angle1);
                    self.angles.sort(function(a, b){return a-b});
                }
                if (link_label2 != '') {
                    link_angle2 = findNewAngle(link_angle2, self.settings.textOffsetAngle);
                    self.angles.push(link_angle2);
                    self.angles.sort(function(a, b){return a-b});
                }
                                                  
                return {
                  link_ori_angle1: link_ori_angle1,
                  link_ori_angle2: link_ori_angle2,
                  link_angle1: link_angle1,
                  link_angle2: link_angle2,
                  link_label1: link_label1,
                  link_label2: link_label2,
                  link_pair: fusion_label,
                  link_width: self.LINKsettings.LinkWidth,
                  link_label1_x1 : (0 + Math.sin(link_ori_angle1) * label1_line_start),
                  link_label1_y1 : (0 - Math.cos(link_ori_angle1) * label1_line_start),
                  link_label1_x2 : (0 + Math.sin(link_angle1) * label1_line_end),
                  link_label1_y2 : (0 - Math.cos(link_angle1) * label1_line_end),
                  link_label2_x1 : (0 + Math.sin(link_ori_angle2) * label2_line_start),
                  link_label2_y1 : (0 - Math.cos(link_ori_angle2) * label2_line_start),
                  link_label2_x2 : (0 + Math.sin(link_angle2) * label2_line_end),
                  link_label2_y2 : (0 - Math.cos(link_angle2) * label2_line_end),
        ...
        for(var linki=0; linki<self.LINK.length; linki++){
            ...
            svg.append("g")
              ...
              .selectAll("text")
                ...
                .enter().append("text")
                .attr("text-anchor", function(d) {
                    return ((d.link_angle1 * 180 / Math.PI - 90) > 90) ? "end" : "start";
                })
                .attr("transform", function(d) {
                    var angle = (d.link_angle1 * 180 / Math.PI - 90);
                    var radius = self.LINKsettings.LinkRadius+self.LINKsettings.LinkAxisPad;
                    console.log("link:" + self.LINKsettings.LinkRadius);
                    return "rotate(" + angle + ")translate(" + ( + (angle > 90 ? ((self.settings.outerRadius*2)+ 110)-radius : radius) )  + ")rotate(" + (angle > 90 ? -180 : 0) + ")";
                    // todo 110 in the above line is a calculation that needs to be figured out... I think
                })
                .attr("class", "BioCircosLINKLabel")
                .attr("id", function(d) { return (Math.round( d.link_angle1 * 10 ) / 10 == Math.round( d.link_angle2 * 10 ) / 10) ? '' : d.link_label1; })
                .attr("x", self.LINKsettings.LinkLabelPad)
                .attr("dy", ".35em")
                .attr("fill", self.LINKsettings.LinkLabelColor)
                .style("font-size",self.LINKsettings.LinkLabelSize)
                //.text(function(d) { return (Math.round( d.link_angle1 * 10 ) / 10 == Math.round( d.link_angle2 * 10 ) / 10) ? '' : d.link_label1; });
                .text(function(d) { return d.link_label1; });
            ...
            .attr("class", "BioCircosLINK")
              .selectAll("path")
                .data(link_objects)
                .enter().append("path")
                .attr("d", function(d) { return "M" + (d.link_label1_x1) +"," + (d.link_label1_y1) +" L" + (d.link_label1_x2) +"," + (d.link_label1_y2) + " M" + (d.link_label2_x1) +"," + (d.link_label2_y1) +" L" + (d.link_label2_x2) +"," + (d.link_label2_y2); })
                .attr("class", "BioCircosLINK")
                .attr("fill","none")
            ...
            svg.append("g")
                ...
                .enter().append("text")
                .attr("text-anchor", function(d) {
                    return ((d.link_angle2 * 180 / Math.PI - 90) > 90) ? "end" : "start";
                })
                .attr("transform", function(d) {
                    var angle = (d.link_angle2 * 180 / Math.PI - 90);
                    var radius = self.LINKsettings.LinkRadius+self.LINKsettings.LinkAxisPad;
                    return "rotate(" + angle + ")translate(" + ( + (angle > 90 ? ((self.settings.outerRadius*2)+ 110)-radius : radius) )  + ")rotate(" + (angle > 90 ? -180 : 0) + ")";
                    // todo 110 in the above line is a calculation that needs to be figured out... I think
                })
                ...
                // })
                .attr("class", "BioCircosLINKLabel")
                .attr("id", function(d) { return (Math.round( d.link_angle1 * 10 ) / 10 == Math.round( d.link_angle2 * 10 ) / 10) ? d.link_pair : d.link_label2; })
                .attr("x", self.LINKsettings.LinkLabelPad)
                .attr("dy", ".35em")
                .attr("fill", self.LINKsettings.LinkLabelColor)
                .style("font-size",self.LINKsettings.LinkLabelSize)
                //.text(function(d) { return (Math.round( d.link_angle1 * 10 ) / 10 == Math.round( d.link_angle2 * 10 ) / 10) ? d.link_pair : d.link_label2; });
                .text(function(d) { return d.link_label2; });
    ...
    if(self.settings.showLegend==true) {

        var legend_x = -Math.abs(self.settings.svgWidth / 2) + 40;
        var legend_y = -Math.abs(self.settings.svgHeight / 2) + 25;

        d3.select(self.target + " > svg > g")
            .append("g")
            .attr("class", "legend");

        for (var n=0; n < self.LEGEND[0].length; n++){

            d3.select(".legend")
                .append("rect")
                .attr("x", legend_x)
                .attr("y", legend_y + (n * 30))
                .attr("width", 25)
                .attr("height", 25)
                .attr("fill", self.LEGEND[0][n][1])
                .attr("rx", 5)
                .style("stroke", "#000")
            ...
            d3.select(".legend")
                .append("text")
                .attr("x", legend_x + 30)
                .attr("y", (legend_y + 12) + (30 * n))
                .attr("text-anchor", "left")
                .style("font-size", "15px")
                .style("font-weight", "bold")
                .text(self.LEGEND[0][n][0]);

            d3.select(".legend")
                .append("text")
                .attr("x", legend_x + 30)
                .attr("y", (legend_y + 24) + (30 * n))
                .attr("text-anchor", "left")
                .style("font-size", "11px")
                .style("font-style", "italic")
                .text(self.LEGEND[0][n][2]);
