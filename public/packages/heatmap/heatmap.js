...
function oncoHeatmap(config) {
  ...
  tooltip = d3.tip()
        ...
        .offset([-10, 0])
        .html(function(d) {                
                return config.getHint(rowLabel[d.row-1], colLabel[d.col-1], d.value);                
  ...
  var rowLabels = svg.append("g")
      ...
      .attr("x", 0)
      .attr("y", function (d, i) { return (hcrow.indexOf(i+1) + 1) * cellSize; })
  ...
  var sample_meta_cells = svg.append("g")
      ...
      .attr("y", 0)
      .attr("class", function(d, i){return "sample_meta c" + i;})
      .attr("width", cellSize)
  ...
  var heatMap = svg.append("g").attr("class","g3")
        ...
        .attr("y", function(d) { return (hcrow.indexOf(d.row) + 1) * cellSize; })
        ...
        .on("mouseover", function(d){
              ...
              var hint_text = config.getHint(rowLabel[d.row-1], colLabel[d.col-1], d.value);
  ...
  function sortbylabel(rORc,i,sortOrder){
       ...
       if(rORc=="r"){ // sort log2ratio of a gene
         ...
         t.selectAll(".rowLabel")
          .attr("y", function (d, i) { return (sorted.indexOf(i)+1) * cellSize; })
         ;
       }
  }
...
oncoHeatmap.prototype.drawMeta = function(title, sample_meta) {
  ...
  svg.append("text")
    .attr("text-anchor", "left")
    .attr("x", 0)
    .attr("y", this.lengendHeight + cellSize * 1.5)
  ...
  smp_legend.append("text")
    .attr("class", "sample_meta_text mono")
    .text(function(d) { return d; })    
    .attr("transform", "translate(-" + this.margin.left + ",+0)")
    .attr("x", function(d, i) { return title_width + legendElementWidth * i + cellSize + padding; })
    .attr("y", this.lengendHeight + cellSize * 1.5)
    .each( function (d) {text_widths.push(this.getComputedTextLength());})
    ;

  d3.selectAll(".sample_meta_text")
      .attr("x", function (d, i) { return title_width + getTotalTextWidth(i) + (cellSize + padding * 2) * (i + 1) - padding; })
      ;

  smp_legend.append("rect")
    .attr("x", function(d, i) { return  title_width + getTotalTextWidth(i) + (cellSize + padding * 2) * i; })
    .attr("y", this.lengendHeight + cellSize*0.8)
