...
function writeDownloadLink(geneID){
        //console.log("clicked...");
...
function MutsNeedlePlot (config) {
    ...
    mutationRefData = config.mutationRefData || -1;          // .json file or dict
    ...
    this.refTotalCategCounts = {};
    this.refCategCounts = {};
    this.refSelectedNeedles = [];
    ...
    var width = this.width = config.width || targetElement.offsetWidth;
    var height = this.height = config.height || targetElement.offsetHeight || 500;
    //console.log("width:" + width);
    //console.log("width:" + config.targetElement);    
    // Color scale & map
    ...
    buttom_padding = 40;
    this.buffer = buffer;
    this.region_height = 16;
    this.bg_height = bg_height;
    this.bg_offset = 0;
    this.region_offset = 3;
    this.buttom_padding = buttom_padding;
    ...
    var y_height = 50;    
    var bg_y = height / 2 - buttom_padding / 2;
    var y = d3.scale.linear()
      .range([bg_y - y_offset, bg_y - y_offset - y_height])
      .nice();
    this.y = y;    

    var y_ref = d3.scale.linear()
      .range([bg_y + bg_height + y_offset, bg_y + bg_height + y_offset + y_height])
      .nice();
    this.y_ref = y_ref;
    //console.log("height:" + height);
    //console.log("scaley:" + JSON.stringify([bg_y - y_offset, bg_y - y_offset - y_height]));
    //console.log("scaley_ref:" + JSON.stringify([bg_y + bg_height + y_offset, bg_y + bg_height + y_offset + y_height]));
    //console.log(JSON.stringify(y.domain()));
    ...
    function brushmove() {
      //return;
        ...
        for (key in Object.keys(self.refTotalCategCounts)) {
            categCounts[key] = 0;
        }
    ...
    /// DRAW
    this.drawNeedles(svg, mutationData, mutationRefData, regionData);
    ...
    self.on("needleSelectionChange", function(edata) {
            ...
            //alert(selection[0] + '-' + selection[1]);
            if (selection[1] - selection[0] > 0 && selection[0] > 0) {
                self.selectionTip.show({left: selection[0], right: selection[1]}, selectionRect.node());
                d3.select(".d3-tip-selection")
                    .transition()
                    .delay(3000)
...
MutsNeedlePlot.prototype.drawLegend = function(svg) {
    ...
    mutCategories = [];
    categoryColors = [];
    allrefcategs = Object.keys(self.refTotalCategCounts); // random order
    allcategs = Object.keys(self.totalCategCounts); // random order
    allcategs = allcategs.concat(allrefcategs);
    ...
    //xplacement = (self.x(domain[1]) - self.x(domain[0])) * 0 + self.x(domain[0]);
    xplacement = self.width - self.right_padding + 30;
    ...
    var ref_sum = 0;
    for (var c in self.refTotalCategCounts) {
        ref_sum += self.refTotalCategCounts[c];
    }

    legendLabel = function(categ) {
        var count = (self.categCounts[categ] || (self.selectedNeedles.length == 0 && self.totalCategCounts[categ]) || 0);
        var ref_count = (self.refCategCounts[categ] || (self.selectedNeedles.length == 0 && self.refTotalCategCounts[categ]) || 0);
        //return  categ + (count > 0 ? ": "+count+" (" + Math.round(count/sum*100) + "%)" : "");
        return  categ + " " + count + "(" + ref_count + ")";
    };
    ...
    var needleHeads = d3.selectAll(".needleHeadGroup");
...
MutsNeedlePlot.prototype.drawRegions = function(svg, regionData) {
    ...
    function draw(regionList) {
      //console.log("x(minCoord):" + x(minCoord));
        ...
        d3.select(".extent")
            .attr("y", y(0) + y_offset - region_offset - 10);
        ...
        regions.append("rect")
            ...
            .attr("y", y(0) + y_offset - region_offset )
            .attr("ry", "3")
            .attr("rx", "3")
        ...
        regions.append("text")
            ...
            .attr("y", function(r) {r.y = y(0) + y_offset + text_offset; return r.y; } )
            .attr("dy", "0.35em")
...
MutsNeedlePlot.prototype.drawYAxis = function(svg, is_ref=false) {
    var y = (is_ref)? this.y_ref: this.y;
    var buttom_padding = this.buttom_padding;
    yAxis = d3.svg.axis().scale(y).orient("left").ticks(2);;
    var x_pos = this.buffer * 1.5;
    ...
    svg.append("text")
        .attr("class", "y-label")
        .attr("text-anchor", "middle")
        .attr("transform", "translate(" + (this.buffer - 20) + "," + ( this.height / 2 - 1.2 * buttom_padding)+ "), rotate(-90)")
        .text("#Mutations")
        .attr('font-size', 11);
    svg.append("text")
        .attr("class", "y-label")
        .attr("text-anchor", "middle")
        .attr("transform", "translate(" + (this.buffer - 20) + "," + (this.height - buttom_padding * 1.8) + "), rotate(-90)")
        .text("Reported")
        .attr('font-size', 11);
    
}

MutsNeedlePlot.prototype.drawXAxis = function(svg) {
    ...
    svg.append("svg:g")
      ...
      .attr("transform", "translate(0," + (this.height - buttom_padding) + ")")
      .call(xAxis);
...
MutsNeedlePlot.prototype.drawNeedles = function(svg, mutationData, mutationRefData, regionData) {

    var y = this.y;
    var y_ref = this.y_ref;
    var x = this.x;
    var self = this;
    var region_height = this.region_height;
    var region_offset = this.region_offset;
    ...
    function formatMutationEntry(d, is_ref) {
        ...
        h = (is_ref)? highest_ref: highest;
        //if (stickHeight + numericValue > h) {
        if (numericValue > h) {
            // set Y-Axis always to highest available
            h = numericValue;
            getYAxis(is_ref).domain([0, h + 2]);
            //getYAxis().domain([-80, highest + 2]);
            //getYAxis(is_ref).domain([transform_value(h) * (-1) - 2, transform_value(h) + 2]);
        }
        ...
        if (numericCoord > 0) {
            ...
            if (is_ref)
              self.refTotalCategCounts[category] = (self.refTotalCategCounts[category] || 0) + numericValue;
            else
    ...
    if (typeof mutationData == "string") {
        ...
        muts = prepareMuts(mutationData, 0);
        mutRefs = prepareMuts(mutationRefData, 1);
        paintMuts(mutRefs, true);
        paintMuts(muts); 
        ...
        self.drawLegend(svg);
        self.drawYAxis(svg);
        self.drawYAxis(svg, true);
        self.drawXAxis(svg);
        
        
    }

    function prepareMuts(unformattedMuts, is_ref) {
        var muts = [];
        for (key in unformattedMuts) {
            formatted = formatMutationEntry(unformattedMuts[key], is_ref);
    ...
    function paintMuts(muts, is_ref=false) {
        ...
        //    .data(muts).enter()
        var y1 = (is_ref)? y_ref(0) - y_offset : y(0) + y_offset;

        needleHeads.append("line")
            .attr("y1", function(data) { return y1 })            
            .attr("x1", function(data) { return x(data.coord) })
            .attr("y2", function(data) { return (is_ref)? y_ref(transform_value(data.value)) - headSize(data.value) : y(transform_value(data.value)) + headSize(data.value); } )
            .attr("x2", function(data) { return x(data.coord) }).attr("class", "needle-line")
            .attr("stroke", "black")
            .attr("stroke-width", 1)
        ...
        needleHeads.append("circle")
            .attr("cy", function(data) { return (is_ref)? y_ref(transform_value(data.value)) : y(transform_value(data.value));} )
            .attr("cx", function(data) { return x(data.coord) } )
            .attr("r", function(data) { return 1 })
            .attr('cursor', 'pointer')
            ...
            .transition()
            .duration(0)
            .attr("r", function(data) { return headSize() })
            .each("end", 
              function(){
                /*
                needleHeads.append("text")            
                  .attr("y", function(data) { return y(0+transform_value(data.value)) - 10 } )
                  .attr("x", function(data) { return x(data.coord) } )
            ...
            needleHeads.attr('pointer-events', 'all')
            .attr('cursor', 'pointer')
            .on("click",  function(data) {self.selectNode(data);tip.hide;})
            .on('mouseout', tip.hide);
...
/*
            ...
            .data(muts)
            .enter().append("text")
            .attr("y", function(data) { return y(data.stickHeight+data.value) } )
            .attr("x", function(data) { return x(data.coord) } )
            .text(function(data) { return data.value } )
            .attr({"alignment-baseline":"middle", "text-anchor":"middle"});
*/
